...
from ...MAYOEstimator.MAYOAlgorithms.reconciliation_fi import ReconciliationFI as ReconciliationFIMAYO
from ...MAYOEstimator.mayo_problem import MAYOProblem


class ReconciliationFI(UOVAlgorithm):
    def __init__(self, problem: UOVProblem, **kwargs):
        """Construct an instance of the Furue-Ikematsu reconciliation attack estimator.

        This is the reconciliation attack of Furue and Ikematsu [FI26]_, carried out over the
        p^ell-truncated polynomial ring R(p^ell) = F_q[x_1, ..., x_n] / <x_1^(p^ell), ..., x_n^(p^ell)>,
        where q = p^e. A UOV public key is a MAYO public key with o = m and k = 1, so the estimate is
        the one of the MAYO algorithm of the same name.
        ...
        Examples:
            >>> from cryptographic_estimators.UOVEstimator.UOVAlgorithms.reconciliation_fi import ReconciliationFI
            >>> from cryptographic_estimators.UOVEstimator.uov_problem import UOVProblem
            >>> E = ReconciliationFI(UOVProblem(n=112, m=44, q=256, theta=None))
            >>> E
            ReconciliationFI estimator for the UOV signature scheme with parameters (q, n, m) = (256, 112, 44)
        """
        ...
        self._E = ReconciliationFIMAYO(MAYOProblem(n=n, m=m, o=m, q=q, k=1), **mayo_kwargs)
        self.set_parameter_ranges("k", 0, self._E._o - 1)
    ...
    def D(self):
        ...
        As on the MAYO algorithm this delegates to, D is derived from (k, ell) by Equation (11) of
        ...
        `MAYOEstimator.MAYOAlgorithms.reconciliation_fi.ReconciliationFI.D`.
