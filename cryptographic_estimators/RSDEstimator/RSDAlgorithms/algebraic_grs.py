...
class AlgebraicGRS(RSDAlgorithm):
    """
    ...
    def __init__(self, problem: RSDProblem, **kwargs):
        super(AlgebraicGRS, self).__init__(problem, **kwargs)

        q, m, n, k, r = self.problem.get_parameters()

        self._name = "AlgebraicGRS"
    ...
    def _compute_time_complexity(self, parameters: dict):
        ...
        q, m, n, k, r = self.problem.get_parameters()
        w = self.w
        val = ceil(((r + 1) * (k + 1) - (n + 1)) / r)
        time_complexity = w * log2(r) + w * log2(k) + r * val * log2(q)
