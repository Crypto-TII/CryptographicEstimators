...
docker-test:
	@docker run --name container-for-test -d -it ${image_name} sh && docker exec container-for-test sage -t --long -T 3600 --nthreads 4 --force-lib cryptographic_estimators/DummyEstimator/dummy_estimator.py  && docker stop container-for-test
